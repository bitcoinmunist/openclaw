...
<AccordionGroup>
  <Accordion title="3. Legacy state migrations (disk layout)">
    ...
    Device identity and other shared file-claim migrations also work with `OPENCLAW_FS_SAFE_NATIVE_MODE=off` (for example, on Android/Termux). They use guarded, same-directory hardlink publication without overwriting an existing claim, verify the original file identity, and recover an interrupted source/claim link pair before retrying. The filesystem must support hardlinks and the required directory synchronization. Keep native mode disabled for the repair if the platform cannot run the native helper, then rerun `openclaw doctor --fix` against the same state/config before restarting the Gateway.
